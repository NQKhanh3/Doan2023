    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col-8">
                        <a href="{{ route('User.create') }}" class="btn btn-success waves-effect waves-light mb-4"><i class="fas fa-plus-square"></i> Thêm mới</a>
                    </div>
                    <div class="col-4 form-search">
                        <form action="{{ route('User.list') }}" method="GET" id="formSearch" class="app-search mt-0">
                        ...
                        <tbody>
                            @if (count($user) > 0)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div>
                                            <a href="{{ route('User.edit', ['id' => $user->id]) }}" class="btn btn-warning btn-sm waves-effect waves-light" data-toggle="tooltip" data-placement="top" title="Chỉnh sửa"><i class="fas fa-edit"></i></a>
                ...
                <div class="d-flex justify-content-between mt-3">
                    <!-- <div style="padding: .5rem .75rem; margin-bottom: 1rem;">
                        Hiển thị @if ($user->firstItem())
                            {{ $user->firstItem() }}
                        @else
                            0
                        @endif tới @if ($user->lastItem())
                            {{ $user->lastItem() }}
                        @else
                            0
                        @endif trong {{ $user->total() }} mục
                    </div> -->
                    <div>
                        {{ $user->onEachSide(1)->withQueryString()->links() }}
                    </div>
                </div>
                @endif
            </div>
